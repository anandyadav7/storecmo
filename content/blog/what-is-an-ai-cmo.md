...
Strip away the positioning and every product in this category is claiming some combination of five things: analysis, prioritisation, strategy, execution, and judgment. They are not equally mature, and the order matters, because each one depends on the one before it.

| Function | The claim | How real it is today |
| --- | --- | --- |
| Analysis | Pulls scattered channel data together and tells you what happened | Strong. This is the most proven part. |
| Prioritisation | Ranks what to fix or fund first, with a reason | Workable, and the most valuable part for a small store. |
| Strategy | Weighs trade-offs against your goals and produces a plan | Patchy. Depends almost entirely on how much of your situation it knows. |
| Execution | Writes, changes, publishes, adjusts spend | Easy to build, widely available, and the part that needs the most supervision. |
| Judgment | Decides what your brand should stand for and what is worth the risk | Not on offer, whatever the marketing says. |
...
Agencies are the comparison most store owners are actually weighing, and the failure mode there is rarely competence. It is fragmentation: the agency sends reports, the email freelancer runs flows, the ads person runs campaigns, and nobody owns the question of what the store should do next. That question is the actual vacancy in most lean ecommerce teams, and it is the one this category is aiming at.
...
- **Is your data connected enough to be read in one place?** Orders, analytics, email, ads. Not perfect, just reachable.
- **Do you have a specific recurring question?** "Do my marketing" is not one. "What should I feature in next week's email, and why" is.
- **Will you act on the answer?** An AI CMO that produces recommendations you never run is an expensive newsletter.
...
- **Which of the five functions does it actually do?** Ask for a demonstration of prioritisation specifically. Most products that claim it show you a dashboard.
- **Can you see the reasoning, or only the conclusion?** A recommendation you cannot interrogate is a recommendation you cannot safely act on.
- **What does it know about your store, and how did it learn it?** Connected data and maintained context, or a prompt box with your brand name in it.
- **Where does it sit on the autonomy ladder, and can you move it?** Any product that cannot be turned down to approval-gated is telling you something.
- **What objective is it optimising, and who set it?** This is the malaria-campaign question. If the metric is wrong, everything downstream is confidently wrong.
...
No, and the sources selling these products say so themselves. CDP.com's position is that an AI CMO "augments human marketing leadership rather than replacing it," with the human setting objectives and guardrails and the AI working inside them. Improvado draws the same line: the system handles data analysis, pattern recognition, and decision support, not the strategic vision, creative judgment, or brand ownership. For a store that has never had a marketing lead, the more accurate framing is that it fills part of a vacancy, not that it replaces a person.
...
Improvado puts typical enterprise implementation at eight to twelve weeks, covering connecting data sources, validating accuracy, configuring governance, and training. A small store will not need most of that, but the underlying dependency does not shrink: time to value is set by how quickly the system can read your data honestly. Fragmented data and unclear KPIs push it out for everyone.
...
Scope. An agent does a task: writing descriptions, adjusting bids, tagging products. An AI CMO coordinates across tasks and decides which ones deserve the resource in the first place. In practice a lot of products marketed as the second are the first with a strategy prompt attached, which is why the demonstration matters more than the description.
